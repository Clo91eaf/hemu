...
impl Exception {
  ...
  pub fn take_trap(&self, cpu: &mut Cpu) -> Trap {
    ...
    if previous_mode <= Mode::Supervisor && ((cpu.csr.read(MEDELEG) >> cause) & 1) == 1 {
      ...
      cpu.csr.write(STVAL, self.trap_value(exception_pc));
      ...
      cpu.pc = (cpu.csr.read(MTVEC) & !1) as u64;
      ...
      // explicitly written by software."
      cpu.csr.write(MTVAL, self.trap_value(exception_pc));
