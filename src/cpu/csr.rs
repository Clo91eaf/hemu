...
/// The number of csr. The field is 12 bits so the maximum kind of csr is 4096 (2**12).
...
/// The state to contains all the csr.
pub struct Csr {
    csr: [u64; CSR_SIZE],
}
...
impl Csr {
    ...
    pub fn increment_time(&mut self) {
        self.csr[TIME as usize] = self.csr[TIME as usize].wrapping_add(1);
    }
    ...
    pub fn read(&self, addr: CsrAddress) -> u64 {
        ...
        // non-existence) of higher privilege levels (machine level or other) visible in the csr
        // accessible by the supervisor.  Many supervisor csr are a subset of the equivalent
        ...
        match addr {
            SSTATUS => self.csr[MSTATUS as usize] & SSTATUS_MASK,
            SIE => self.csr[MIE as usize] & self.csr[MIDELEG as usize],
            SIP => self.csr[MIP as usize] & self.csr[MIDELEG as usize],
    ...
    pub fn write(&mut self, addr: CsrAddress, val: u64) {
        ...
        match addr {
            ...
            MHARTID => {}
            SSTATUS => {
                self.csr[MSTATUS as usize] =
                    (self.csr[MSTATUS as usize] & !SSTATUS_MASK) | (val & SSTATUS_MASK);
            }
            SIE => {
                self.csr[MIE as usize] = (self.csr[MIE as usize] & !self.csr[MIDELEG as usize])
                    | (val & self.csr[MIDELEG as usize]);
            }
            SIP => {
                let mask = SSIP_BIT & self.csr[MIDELEG as usize];
                self.csr[MIP as usize] = (self.csr[MIP as usize] & !mask) | (val & mask);
            }
            _ => self.csr[addr as usize] = val,
        }
    }
